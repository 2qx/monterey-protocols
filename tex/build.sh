#!/usr/bin/bash

pdflatex monterey-protocols.tex;

pdfjam cover.pdf monterey-protocols.pdf back.pdf -o monterey-protocols-A5.pdf;

cp monterey-protocols-A5.pdf ../assets/monterey-protocols_A5.pdf;

pdfjam monterey-protocols.pdf --booklet true --paper a4 --landscape --outfile monterey-protocols-zine.pdf;
pdfjam monterey-protocols-zine_cover.pdf '1,{}' monterey-protocols-zine.pdf --paper a4 --landscape  --outfile ../assets/monterey-protocols-zine.pdf;